...
describe('AIPromptTemplates', () => {
  ...
  describe('Plugin Generation Prompts', () => {
    ...
    it('should enhance requirements with technical context', () => {
      ...
      expect(userPrompt).toContain('command handling with proper permission checks');
      expect(userPrompt).toContain('event listeners with proper event handling');
  ...
  describe('Prompt Enhancement', () => {
    it('should generate comprehensive enhancement system prompt', () => {
      ...
      expect(systemPrompt).toContain('expert Minecraft plugin development consultant');
    ...
    it('should generate context-aware enhancement user prompt', () => {
      ...
      expect(userPrompt).toContain('brief requirement that needs significant expansion');
    });
  ...
  describe('Parameter Validation', () => {
    it('should validate valid parameters', () => {
      const result = service.validatePromptParameters('TestPlugin', 'Create a simple plugin');
    ...
    it('should reject invalid plugin name format', () => {
      const result = service.validatePromptParameters('123Invalid', 'Create a plugin');
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Plugin name must start with a letter and contain only letters, numbers, underscores, and hyphens');
    });
    ...
    it('should reject too long parameters', () => {
      ...
      expect(nameResult.errors).toContain('Plugin name is too long (max 100 characters)');
      
      const reqResult = service.validatePromptParameters('Test', longRequirements);
      expect(reqResult.isValid).toBe(false);
      expect(reqResult.errors).toContain('Requirements are too long (max 50000 characters)');
    });
  });

  describe('Fallback Project Generation', () => {
    it('should generate fallback project template', () => {
      const project = service.getFallbackProjectTemplate('TestPlugin', 'Create a test plugin');
      ...
      expect(filePaths.some(path => path.includes('TestPluginPlugin.java'))).toBe(true);
    ...
    it('should generate valid plugin.yml content', () => {
      ...
      expect(yamlFile!.content).toContain('main: com.example.testplugin.TestPluginPlugin');
