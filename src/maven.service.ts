...
export class MavenService {
  ...
  async compilePlugin(projectPath: string): Promise<CompilationResult> {
    ...
    console.log(
      `🔨 Maven Service: Starting compilation for project at: ${projectPath}`,
    );
    ...
    try {
      ...
      if (!(await fs.pathExists(projectPath))) {
        console.log(
          `❌ Maven Service: Project directory not found: ${projectPath}`,
        );
        return {
          success: false,
      ...
      console.log(
        `📝 Maven Service: stdout length: ${stdout.length} characters`,
      );
      if (stderr) {
        console.log(
          `⚠️ Maven Service: stderr length: ${stderr.length} characters`,
        );
      }
      ...
      if (jarFiles.length === 0) {
        console.log(
          `❌ Maven Service: No JAR files generated despite compilation success`,
        );
        return {
          success: false,
  ...
  async compilePluginByUserAndName(
    userId: string,
    pluginName: string,
    ...
    console.log(
      `🔨 Maven Service: Compiling plugin for user "${userId}", plugin "${pluginName}"`,
    );
  ...
  private async findJarFiles(targetDir: string): Promise<string[]> {
    ...
    try {
      ...
      console.log(
        `📦 Maven Service: Found JAR files: ${jarFiles.map((f) => path.basename(f)).join(', ')}`,
      );
      return jarFiles;
    } catch (error) {
  ...
  async getCompilationStatus(projectPath: string): Promise<{
    ...
    console.log(
      `📊 Maven Service: Checking compilation status for: ${projectPath}`,
    );
    try {
      const targetDir = path.join(projectPath, 'target');
      ...
      console.log(
        `📦 Maven Service: JAR files found: ${hasJar} (${jarFiles.length} files)`,
      );
      ...
      if (hasJar) {
        ...
        console.log(
          `⏰ Maven Service: Last JAR modification: ${lastModified.toISOString()}`,
        );
      }
      ...
      console.error(
        '❌ Maven Service: Error checking compilation status:',
        error,
      );
      return {
        hasTarget: false,
  ...
  async cleanProject(projectPath: string): Promise<boolean> {
    ...
    try {
      ...
      if (await fs.pathExists(targetDir)) {
        ...
        console.log(
          `ℹ️ Maven Service: Target directory doesn't exist, nothing to clean`,
        );
        return true;
      }
    } catch (error) {
