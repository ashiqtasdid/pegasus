...
export class AppService {
  ...
  async generatePlugin(
    ...
    console.log(`🎯 App Service: Complexity level: ${complexity}/10`);
    ...
    try {
      ...
      if (projectExists) {
        console.log(
          `🔄 App Service: Project "${pluginName}" already exists, recompiling instead of recreating...`,
        );
        ...
        if (await fs.pathExists(pomPath)) {
          console.log(
            `✅ App Service: Found existing project with pom.xml, proceeding with recompilation`,
          );
          ...
          if (cleanSuccess) {
            ...
            console.log(
              `⚠️ App Service: Project clean failed, continuing anyway`,
            );
          }
          ...
          const compilationResult =
            await this.mavenService.compilePlugin(pluginPath);

          const duration = Date.now() - startTime;

          if (compilationResult.success) {
            console.log(
              `🎉 App Service: Existing project recompiled successfully in ${duration}ms`,
            );
            ...
            console.log(
              `❌ App Service: Existing project compilation failed in ${duration}ms`,
            );

            // If recompilation fails, we could try to fix it or fall back to regeneration
            console.log(
              `🔄 App Service: Recompilation failed, attempting to fix errors with AI...`,
            );
            try {
              const fixResult = await this.errorFixService.attemptErrorFix(
                userId,
                pluginName,
                5,
              );
              ...
              if (fixResult.success) {
                console.log(
                  `🎉 App Service: Existing project fixed and compiled successfully`,
                );
                ...
                console.log(
                  `❌ App Service: Failed to fix existing project, will regenerate instead`,
                );
                // Fall through to regenerate the project
              }
            } catch (fixError) {
              console.log(
                `❌ App Service: Error during fix attempt: ${fixError.message}, will regenerate instead`,
              );
              // Fall through to regenerate the project
            }
          }
          ...
          console.log(
            `⚠️ App Service: Directory exists but no pom.xml found, treating as incomplete project`,
          );
          // Fall through to regenerate the project
        }
        ...
        console.log(
          `🗑️ App Service: Removing incomplete/failed project directory...`,
        );
        try {
          ...
          console.log(
            `⚠️ App Service: Failed to remove old directory: ${removeError.message}`,
          );
        }
      }
      ...
      console.log(
        `🔧 App Service: Enhancing prompt with AI (complexity: ${complexity})...`,
      );
      ...
      console.log(
        `✅ App Service: Prompt enhanced (${enhancedPrompt.length} chars)`,
      );

      // Generate plugin project using AI
      console.log(
        `🤖 App Service: Generating plugin code with AI (complexity: ${complexity})...`,
      );
      const pluginProject = await this.aiService.generatePluginCode(
        enhancedPrompt,
        pluginName,
        complexity,
      );
      console.log(
        `✅ App Service: AI generated ${pluginProject.files.length} files`,
      ...
      for (const file of pluginProject.files) {
        ...
        console.log(
          `📄 App Service: Writing file: ${file.path} (${file.content.length} chars)`,
        );
      ...
      console.log(
        `✅ App Service: All ${pluginProject.files.length} files written to disk`,
      );
      ...
      const nameInfo = name
        ? ` with name: ${name}`
        : ` with auto-generated name: ${pluginName}`;
      const filesList = pluginProject.files
        .map((f) => `- ${f.path} (${f.type})`)
        .join('\n');
      console.log(
        `🎉 App Service: Plugin generation completed in ${duration}ms`,
      );
      console.log(
        `📦 App Service: Generated project "${pluginProject.projectName}" for Minecraft ${pluginProject.minecraftVersion}`,
      );

      // Sync with MongoDB in background
      this.syncPluginWithMongoDb(
        userId,
        pluginName,
        pluginPath,
        pluginProject,
        ...
        console.warn(
          `⚠️ App Service: MongoDB sync failed (continuing anyway): ${error.message}`,
        );
      });
      ...
      const duration = Date.now() - startTime;
      console.error(
        `❌ App Service: Plugin generation failed after ${duration}ms:`,
        error,
      );
  ...
  async generateCodeOnly(
    ...
    console.log(
      `🧩 App Service: Generating code-only for plugin "${pluginName}"`,
    );
    ...
    try {
      ...
      console.log(
        `✅ App Service: Code generation completed - ${pluginProject.files.length} files`,
      );
  ...
  async generateAndCompilePlugin(
    ...
    console.log(
      `🚀 App Service: Starting full plugin generation and compilation workflow`,
    );
    console.log(
      `👤 App Service: User: "${userId}", Plugin: "${name || '(auto-generated)'}", Compile: ${shouldCompile}, Complexity: ${complexity}/10`,
    );

    // First generate the plugin
    console.log(`📝 App Service: Step 1 - Generating plugin files...`);
    const generationResult = await this.generatePlugin(
      prompt,
      userId,
      name,
      complexity,
    ...
    const pluginName = name || `plugin_${Date.now()}`;
    console.log(
      `🔨 App Service: Step 2 - Starting automatic compilation for plugin: ${pluginName}`,
    );

    let compilationResult = await this.mavenService.compilePluginByUserAndName(
      userId,
      pluginName,
    );
    let fixAttempts = 0;
    ...
    while (!compilationResult.success && fixAttempts < maxFixAttempts) {
      fixAttempts++;
      console.log(
        `❌ App Service: Compilation failed, attempting AI fix (attempt ${fixAttempts}/${maxFixAttempts})`,
      );
      ...
      try {
        ...
        if (fixResult.success) {
          console.log(
            `✅ App Service: Auto-fix successful on attempt ${fixAttempts}`,
          );
          compilationResult = fixResult.finalCompilationResult!;
          break;
        } else {
          console.log(
            `❌ App Service: Auto-fix failed on attempt ${fixAttempts}: ${fixResult.message}`,
          );
        }
      } catch (error) {
        console.error(
          `❌ App Service: Error during auto-fix attempt ${fixAttempts}:`,
          error.message,
        );
      }
      ...
      if (fixAttempts < maxFixAttempts) {
        ...
        compilationResult = await this.mavenService.compilePluginByUserAndName(
          userId,
          pluginName,
        );
      }
    ...
    console.log(
      `📊 App Service: Compilation workflow completed after ${fixAttempts} fix attempts`,
    );
    let compilationInfo = '';
    if (compilationResult.success) {
      const fixInfo =
        fixAttempts > 0
          ? ` (fixed after ${fixAttempts} auto-fix attempts)`
          : '';
      compilationInfo = `\n\n🎉 COMPILATION SUCCESSFUL${fixInfo}!\nJAR file created: ${compilationResult.jarPath}\nThe plugin is ready to deploy to your Minecraft server!`;
      console.log(
        `🎉 App Service: Plugin ${pluginName} compiled successfully${fixInfo}`,
      );
    } else {
      ...
      console.error(
        `❌ App Service: Plugin ${pluginName} compilation failed after ${fixAttempts} fix attempts:`,
  ...
  async generatePluginOnly(
    ...
    try {
      ...
      const pluginName = name || `plugin_${Date.now()}`;
      console.log(
        `📝 App Service: Generate-only mode for "${pluginName}" (complexity: ${complexity}/10)`,
      );
  ...
  async checkProjectExists(
    ...
    console.log(
      `🔍 App Service: Checking if project exists - userId: "${userId}", pluginName: "${pluginName}"`,
    );
    ...
    try {
      ...
      const compilationStatus =
        await this.mavenService.getCompilationStatus(projectPath);
      ...
      try {
        ...
        console.log(
          `⚠️ App Service: Could not get project stats: ${error.message}`,
        );
      }

      console.log(
        `📊 App Service: Project exists with JAR: ${compilationStatus.hasJar}`,
      );
  ...
  async attemptErrorFix(
    ...
    console.log(
      `📊 App Service: Parameters - userId: "${userId}", pluginName: "${pluginName}", maxIterations: ${maxIterations}`,
    );

    const startTime = Date.now();
    const result = await this.errorFixService.attemptErrorFix(
      userId,
      pluginName,
      maxIterations,
    );
    ...
    console.log(
      `${result.success ? '✅' : '❌'} App Service: Error fix ${result.success ? 'completed' : 'failed'} in ${duration}ms`,
    );
    console.log(
      `📊 App Service: Fix result - fixAttempted: ${result.fixAttempted}, iterations: ${result.iterations}, operations: ${result.operationsApplied || 0}`,
    );
  ...
  private async safeWriteFile(
    ...
    try {
      ...
      try {
        ...
        console.log(
          `✅ App Service: Successfully wrote file: ${filePath} (${content.length} chars)`,
        );
      } catch (writeError) {
      ...
      console.error(
        `❌ App Service: Error writing file ${filePath}: ${error.message}`,
      );
  ...
  private safeJSONStringify(object: any, indent: number = 2): string {
    try {
      ...
      console.warn(
        `⚠️ App Service: JSON stringification failed: ${error.message}`,
      );
      return '{}';
    }
  }
  ...
  private async syncPluginWithMongoDb(
    ...
    console.log(
      `🗄️ App Service: Syncing plugin with MongoDB - user: "${userId}", plugin: "${pluginName}"`,
    );

    try {
      // Extract metadata from plugin project
      const pluginDto = {
        ...
        metadata: {
          author: 'Pegasus AI',
          version: '1.0.0',
          mainClass: this.extractMainClassFromFiles(
            pluginProject.files,
            pluginName,
          ),
      ...
      console.error(
        `❌ App Service: Failed to sync plugin with MongoDB: ${error.message}`,
        error.stack,
      );
  ...
  private generatePluginId(userId: string, pluginName: string): string {
    ...
    const uniqueString = `${userId}:${pluginName}:${timestamp}:${random}`;
   ...
   */
  private extractMainClassFromFiles(
    files: PluginFile[],
    pluginName: string,
  ): string {
    ...
    const mainFile = files.find(
      (f) =>
        ...
        (f.path.toLowerCase().includes('plugin') ||
    ...
    if (mainFile) {
      ...
      const classMatch = mainFile.content.match(
