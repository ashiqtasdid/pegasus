...
describe('AiService', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: {
            getPluginGenerationSystemPrompt: jest
              .fn()
              .mockReturnValue('System prompt for plugin generation'),
            getPluginGenerationUserPrompt: jest
              .fn()
              .mockReturnValue('User prompt for TestPlugin'),
            getPromptEnhancementSystemPrompt: jest
              .fn()
              .mockReturnValue('System prompt for enhancement'),
            getPromptEnhancementUserPrompt: jest
              .fn()
              .mockReturnValue('Enhanced prompt'),
  ...
  describe('generatePluginCode', () => {
    it('should generate plugin code successfully', async () => {
      const result = await service.generatePluginCode(
    ...
    it('should validate parameters before generation', async () => {
      ...
      expect(promptTemplates.validatePromptParameters).toHaveBeenCalledWith(
        'TestPlugin',
        'Create a test plugin',
      );
    });
    ...
    it('should use fallback when JSON parsing fails', async () => {
      ...
      expect(result).toBeDefined();
      expect(promptTemplates.getFallbackProjectTemplate).toHaveBeenCalledWith(
        'TestPlugin',
        'Create a test plugin',
      );
    });
    ...
    it('should throw error for invalid parameters', async () => {
      (
        promptTemplates.validatePromptParameters as jest.Mock
      ).mockReturnValueOnce({
        isValid: false,
        errors: ['Plugin name is required'],
      });

      await expect(service.generatePluginCode('', '')).rejects.toThrow(
        'Invalid parameters: Plugin name is required',
      );
    });
  });
      ...
      const validProject = {
        ...
        files: [
          ...
          },
          {
            path: 'pom.xml',
            content:
              '<project><dependencies><dependency><groupId>org.spigotmc</groupId><artifactId>spigot-api</artifactId></dependency></dependencies></project>',
            type: 'xml',
          },
          {
            path: 'README.md',
